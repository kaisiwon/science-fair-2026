...
### New Game Ideas
- [ ] **Math Dash**
- [ ] **🍕 Pizza Fraction Parlour (Visual Sorting)**: Customers order pizza slices in fractions (e.g., half pepperoni, three-quarters cheese).Drag and drop pre-cut slices onto the pizza tray to match the order.Score multipliers are awarded for fulfilling orders quickly without wasting ingredients.Visual indicators show fraction circles to keep the game completely intuitive.Levels progress from halves and quarters to eighths and thirds.
- [ ] **🦕 Number Nomad (Infinite Runner)**: A character runs forward automatically across a path filled with obstacles.Obstacles are marked with numbers, and your character holds a target number.Jump onto platforms containing factors or multiples that help hit the target.Collect positive numbers to grow bigger and negative numbers to shrink down.Power-ups temporarily freeze the screen to let you choose the right path safely.
- [ ] **🧬 Merge Matrix (2048-Style Puzzle)**: Swipe to slide numbered tiles across a grid inside a laboratory setting.Identical numbers merge together to add up (e.g., 2 merges with 2 to become 4).Special "Operator" tiles (like +1 or ×2) modify rows they pass through.Achieve specific target numbers to synthesize new elements and clear the board.Simple mouse clicks or arrow keys make it instantly playable for anyone.
- [ ] **⚖️ Balance Island (Physics Weight Puzzle)**: Place different quirky animals on a seesaw to balance it perfectly.Animals have simple, clear weight values indicated by size or direct numbers.Balance a heavy elephant on one side using multiple smaller penguins on the other.Solve puzzles by keeping the scale perfectly level before the timer runs out.Visual physics clues help players solve equations without feeling like they are doing math.
